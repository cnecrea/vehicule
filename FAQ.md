...
```yaml
automation:
  - alias: "Notificare RCA pe expira"
    trigger:
      platform: template
      value_template: "{{ state_attr('sensor.vehicul_rca_expiry', 'days_remaining') | int(0) <= 30 }}"
    action:
      service: notify.notify
      data:
        message: "RCA al vehiculului expira in {{ state_attr('sensor.vehicul_rca_expiry', 'days_remaining') }} zile"
```

### Cum creez o notificare pentru expirarea ITP?
Similar cu RCA, dar utilizați `sensor.vehicul_itp_expiry`.
  ...
  - alias: "Actualizare km din GPS"
    ...
    action:
      ...
      data:
        ...
        km_curent: "{{ state_attr('sensor.gps_distanta', 'value') }}"
```
