"""
Modul de licențiere pentru integrarea Vehicule.
...
class LicenseManager:
    """Gestionează licența pentru integrarea Vehicule.
    ...
    async def async_load(self) -> None:
        ...
        if self._status_token:
            ...
            _LOGGER.debug(
                "[Vehicule:License] Cache restaurat: status=%s, cache_valid=%s",
                cached_status,
                cache_valid,
            )
        else:
            _LOGGER.debug("[Vehicule:License] Niciun cache de status — prima rulare")

        # Verifică statusul la server (prima verificare la startup)
        _LOGGER.debug("[Vehicule:License] Verific statusul la server (startup)...")
    ...
    async def _async_save(self) -> None:
        ...
        _LOGGER.debug("[Vehicule:License] Salvez datele în storage")
    ...
    def _generate_fingerprint(self) -> str:
        ...
        - Salt: face fingerprint-ul specific integrării Vehicule
        """
        componente: list[str] = []
    ...
    async def async_check_status(self) -> dict[str, Any]:
        ...
        if self._is_status_cache_valid():
            _LOGGER.debug(
                "[Vehicule:License] Cache valid — folosesc token existent "
        ...
        _LOGGER.debug(
            "[Vehicule:License] Cache expirat sau inexistent — "
        ...
        try:
            ...
            _LOGGER.error(
                "[Vehicule:License] eroare de rețea la verificare status — %s", err
            )
            return self._status_token
        except Exception as err:  # noqa: BLE001
            _LOGGER.error(
                "[Vehicule:License] eroare neașteptată la verificare status — %s", err
            )
            return self._status_token
    ...
    def is_licensed(self) -> bool:
        ...
        if not self._verify_token_signature(token):
            _LOGGER.warning("[Vehicule:License] semnătură token activare invalidă")
            return False

        # Verifică fingerprint-ul din token-ul de activare
        if token.get("fingerprint") != self._fingerprint:
            _LOGGER.warning("[Vehicule:License] fingerprint token nu se potrivește")
            return False

        # Verifică expirarea din token-ul de activare (licențe anuale)
        expires_at = token.get("expires_at")
        if expires_at and time.time() > expires_at:
            _LOGGER.info("[Vehicule:License] licența a expirat (token local)")
            return False
        ...
        if self._status_token and self._is_status_cache_valid():
            ...
            if server_status not in ("licensed", "trial"):
                _LOGGER.warning(
                    "[Vehicule:License] serverul raportează status '%s' — licență invalidă",
                    server_status,
                )
                return False
        ...
        if self._status_token and not self._is_status_cache_valid():
            _LOGGER.warning(
                "[Vehicule:License] cache-ul de status a expirat — necesită "
    ...
    async def async_activate(self, license_key: str) -> dict[str, Any]:
        ...
        try:
            ...
            _LOGGER.error(
                "[Vehicule:License] eroare de rețea la activare — %s", err
            )
            return {"success": False, "error": "network_error"}
        except Exception as err:  # noqa: BLE001
            _LOGGER.error(
                "[Vehicule:License] eroare neașteptată la activare — %s", err
            )
    ...
    async def async_notify_event(self, action: str) -> None:
        ...
        try:
            ...
            _LOGGER.debug(
                "[Vehicule:License] Nu s-a putut raporta '%s': %s",
                action, err,
            )
    ...
    async def _async_reload_entries(self) -> None:
        """Reîncarcă toate entry-urile vehicule după activare/dezactivare.
        ...
        _LOGGER.info(
            "[Vehicule:License] Reîncarc %d entry-uri după schimbarea licenței",
            len(entries),
        )
    ...
    def _verify_token_signature(self, token: dict[str, Any]) -> bool:
        ...
        try:
            ...
            _LOGGER.debug(
                "[Vehicule:License] verificare semnătură eșuată — %s", err
            )
            return False
